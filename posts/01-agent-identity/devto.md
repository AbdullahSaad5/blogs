---
title: Why my AI agent kept writing to the wrong client's Salesforce
published: true
description: An LLM with a memory can't be trusted to remember which account it's working for. Here's the bug across client orgs, and the session-scoped identity fix that stopped it.
tags: ai, agents, programming, security
series: Agents that take real actions
cover_image: https://dev-to-uploads.s3.us-east-2.amazonaws.com/uploads/articles/ud9swre0dq8ubvagfs66.png
canonical_url:
---
...
![Action-selection funnel: from a request, resolve the app by name or metadata search, filter to the user's connected apps, then branch on how many match: one use it, many ask which, none ask to connect](https://mermaid.?type=png&bgColor=FFFFFF)
...
| ---------------- | ------------------------------------ | ------------------------------------------ |
| Speed            | Fires immediately                    | Slower, you review first                   |
| Confirmation     | None                                 | Plan review + every destructive step       |
| Best for         | Low stakes, you already stated intent | High stakes, or untrusted content          |
| If it picks wrong | It runs                              | You catch it before it runs                |
...
![The agent runs as a restricted user holding only the alias salesforce-1. It passes the alias plus session id across an OS permission boundary to a privileged worker, which reads the mapper file, resolves the alias to the real id for this session only, and runs on the account the user actually meant](https://mermaid.?type=png&bgColor=FFFFFF)

The agent never appears in that second half of the diagram. It hands over an alias and a session id, and the part that knows the real id lives behind a permission boundary it can't cross.
